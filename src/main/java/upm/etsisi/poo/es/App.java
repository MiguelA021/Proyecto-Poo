...
public class App {
    ...
    private void end() throws Exception {
        ...
        ProductController.getInstance().saveInventory(csvPrinter);
        csvPrinter.printRecord("Cashiers");
        CashierController.getInstance().saveCashiers(csvPrinter);
        csvPrinter.printRecord("Customers");
    ...
    public void userCommand(Completer completer) {
            ...
            public AttributedString highlight(LineReader reader, String buffer) {
                ...
                for (int i = 0; i < parts.length; i++) {
                    ...
                    if (i == 0) {
                        ...
                        if (rootCommand.equals("client") || rootCommand.equals("cash") || rootCommand.equals(("ticket"))
                                || (rootCommand.equals("prod"))) {
                            sb.style(AttributedStyle.DEFAULT.foreground(AttributedStyle.YELLOW));
                        } else {
                            sb.style(AttributedStyle.DEFAULT.foreground(AttributedStyle.RED));
                        }
        ...
        try {
            ...
            LineReader reader = LineReaderBuilder.builder().terminal(terminal).completer(completer).highlighter(myHighlighter)
                    .variable(LineReader.HISTORY_FILE, Paths.get("record_poo.txt")).variable(LineReader.HISTORY_SIZE, 50).build();
    ...
    private void renovate(CSVParser csvParser) throws IOException {
        ...
        int[] ticket = new int[2]; // el primer int es el id del ticket, el segundo es el estado al que se tiene
        // que dejar{0:EMPTY, 1:OPEN, 2:CLOSE}

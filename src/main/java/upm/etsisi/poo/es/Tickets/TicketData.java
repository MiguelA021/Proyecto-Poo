...
public class TicketData {
    ...
    public void csvTickets(CSVParser csvParser) {
        for (CSVRecord record : csvParser) {
            if (!record.get(0).equals("EnterpriseMixedTicket") && !record.get(0).equals("CustomerTicket") && !record.get(0).equals("EnterpriseServiceTicket")) {
                ...
                switch (record.get(0)){
                    case "CustomerTicket":
                        CustomerTicket c = new CustomerTicket(Integer.parseInt(record.get(1)), Status.valueOf(record.get(2)));
                        this.tickets.put(c.getId(), c);
                        break;
                    case "EnterpriseMixedTicket":
                        EnterpriseMixedTicket m = new EnterpriseMixedTicket(Integer.parseInt(record.get(1)), Status.valueOf(record.get(2)));
                        this.tickets.put(m.getId(), m);
                        break;
                    case "EnterpriseServiceTicket":
                        EnterpriseServiceTicket s = new EnterpriseServiceTicket(Integer.parseInt(record.get(1)), Status.valueOf(record.get(2)));
                        break;
                    default:

...
public class Product {
  ...
  public Product(int id, String name, type category, double price, int maxPer) {
    this.name = name;
    this.price = price;
    this.id = id;
  ...
  // The three methods under that sentence are using to referencing the Food and
  // Meeting methods
  ...
  public static class Food extends Product {
    ...
    public Food(int id, String name, double price, String expiryDate, int maxPer) {
      super(id, name, price);
      this.expiryDate = expiryDate;
      MAXPARTICIPANTS = maxPer;
    }

    public int getAdvanceMinHours() { // Se encarga del minimo timepo para que se pueda planificar tanto FOOD como
                                      // MEETING
    ...
    public boolean getStateFood(int numberParticipant) {
      ...
      double plannedHours = ((yearOfExpire - yearOfNow) * 12 * 31 * 24) + ((monthOfExpire - monthOfNow) * 31 * 24)
          + ((daysOfExpire - dayOfNow) * 24);
      return advanceMinHours >= plannedHours && numberParticipant <= numMaxParticipant;
    }

    public String toString() {
      return "{class:Product, id: " + getId() + ", name: '" + getName() + ", price: " + getPrice() + ", expiry date: "
          + expiryDate + " }";
    }
  ...
  public static class Meeting extends Product {
    ...
    public Meeting(int id, String name, double price, String expiryDate, int maxPer) {
      super(id, name, price);
      this.expiryDate = expiryDate;
      this.MAXPARTICIPANTS = maxPer;
    }
